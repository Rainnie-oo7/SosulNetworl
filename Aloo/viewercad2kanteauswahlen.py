...
class CADSoftware:
    ...
    def init_3d_plot(self):
        ...
        self.canvas_3d.mpl_connect('pick_event', self.on_pick)
    ...
    def plot_cuboid(self, x, y, z):
        ...
        for i, line in enumerate(lines):
            self.ax.plot3D(*zip(*vertices[line]), color='b', picker=5, label=f'Edge {i}')

        self.canvas_3d.draw()

    def select_edge(self):
        # Dummy-Funktion für die Auswahl von Kanten
        self.selected_edge = self.canvas_3d.get_tk_widget().event.get_ind()
        self.highlight_selected_edge()

    def on_pick(self, event):
        # Handler für Pick-Events (Kanten auswählen)
        self.selected_edge = event.ind
        self.highlight_selected_edge()

    def highlight_selected_edge(self):
        # Betone die ausgewählte Kante farblich
        for line in self.ax.lines:
            line.set_color('b')  # Setze alle Linien auf die Standardfarbe

        if self.selected_edge is not None:
            for edge_index in self.selected_edge:
                self.ax.lines[edge_index].set_color('r')  # Betone die ausgewählte Kante
